                    ...
                    <div class="px-6">
                        <div class="flex flex-wrap justify-center">
                            <div class=" px-4 lg:order-2 flex justify-center">
                                <div class="h-32 w-32 mt-4">
                                    <img alt="..." src="{{$fixerData[0]->photo_url}}"
                                         class="rounded-full object-cover h-full w-full shadow-lg border-4 border-white">
                                </div>
                            </div>
                            <div class="flex w-full lg:w-4/12 px-4 lg:order-3 lg:text-center lg:self-center">
                                <div class="pl-20 mt-32 sm:mt-0">
                                    Įvertis
                                    @if($rating == 1)
                                ...
                                <div class="pt-4 pb-2">
                                    @if(session('id_repairman') == null && session('id_user') != null)
                                        <a href="/addOrder/{{$fixerData[0] -> repairmans_id}}"
                                           class="bg-indigo-600 px-6 py-2 mr-2 rounded text-white hover:bg-indigo-500">Pasirinkti</a>
                                    @endif
                                    @if(session('id_repairman') == $fixerData[0]->repairmans_id || session('role') == 'admin')
                                        <a href="/fixerProfileEdit/{{$fixerData[0] -> repairmans_id}}"
                                           class="bg-indigo-600 px-6 py-2 ml-2 rounded text-white hover:bg-indigo-500">Redaguoti</a>
                                    @endif
                                </div>
                            </div>
                        </div>
