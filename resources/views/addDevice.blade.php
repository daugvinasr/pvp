...
@section('content')
    <div class="h-screen bg-gray-100 justify-center">
        <div class="py-6 px-8 h-120 w-max mt-20 bg-white rounded shadow-xl">
            <h1 class="text-2xl text-gray-800 font-bold mb-5 mx-20">Naujo įrenginio informacija:</h1>
            <form method="POST">
                @csrf
                ...
                <div class="mb-6">
                    <label for="specifications" class="block text-gray-800 font-bold">Specifikacijos:</label>
                    <input name="specifications" type="text"  class="w-full border border-gray-300 py-2 pl-3 rounded mt-2 outline-none focus:ring-indigo-600 :ring-indigo-600" />
                    @error('specifications')
                    {{ $message }}
                    @enderror
                </div>

                <button class="cursor-pointer py-2 px-4 block mt-6 bg-indigo-500 text-white font-bold w-full text-center rounded " type="submit">Sukurti įrenginį</button>
            </form>
        </div>
    </div>
@endsection
